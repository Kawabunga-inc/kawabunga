...
function asChronicle(value: unknown): JournalChronicle | null {
  ...
  return {
    ...
    timed: (Array.isArray(raw.timed) ? raw.timed : [])
      .map((e) => {
        const r = asRecord(e);
        const afterSeconds = num(r?.afterSeconds);
        const direction = str(r?.direction);
...
function asSceneState(value: unknown): SceneStateLike | null {
  const raw = asRecord(value);
  if (!raw) return null;
  return {
    beat: str(raw.beat),
...
export type JournalDecisionItem = {
  ...
  previousState: SceneStateLike | null;
  nextState: SceneStateLike | null;
  decisionRaw: unknown;
};
...
/** Parse the journal out of a session's event rows — non-journal event
 *  types pass through untouched (they stay visible in the Raw tab). */
