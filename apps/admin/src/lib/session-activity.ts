...
/**
 * The canonical sessions-index activity rule. A persisted `active` status is
 * necessary but not sufficient: at least one session, event, or turn timestamp
 * must also be less than 60 seconds old. The strict comparison deliberately
 * makes the exact 60-second boundary stale.
 */
