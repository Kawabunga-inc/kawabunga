...
${buildActingChannelConvention(characterName)} You do not break character. Your language matches the Voice Identity section exactly — register, idiom, beliefs, taboos.
...
function buildStructuredParts(
  ...
  You are ${characterName}. You do not break character.
</identity>`;
  const performance = `<performance>
  ${buildActingChannelConvention(characterName)}
</performance>`;
  ...
  - Do not bullet-list. Do not number.
  - Apart from the required *asterisks* around an action, do not use markdown formatting in your replies. No **bold**, no headers, no bullet lists, no URLs as clickable links. Speak in plain prose. If you must surface a phone number or external resource, mention it in the flow of the sentence ("call 988"), not as a formatted block.
  ...
  // Order: identity → performance → voice → directive → delivery → context_handling.
  // Identity anchors the model. Performance separates action from speech.
  // Voice modulates HOW it speaks. Directive
  ...
  const cached = [identity, performance, voiceXml, directiveXml, delivery, contextHandling]
    .filter(Boolean)
    .join("\n\n");
...
${buildActingChannelConvention(characterName)} You do not break character.
...
function buildStructuredVoiceParts(
  ...
  const cached = [identity, performance, voiceXml, directiveXml, delivery].filter(Boolean).join("\n\n");
